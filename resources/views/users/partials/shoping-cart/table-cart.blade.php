            ...
            @forelse($cartItems as $item)
                <tr data-id="{{ $item->id_sp_giohang }}">
                    <td class="shoping__cart__item">
                        <img src="{{ asset($item->product->thumbnail) }}" alt="{{ $item->product->tensanpham }}"
                            width="100">
                        ...
                        <div class="quantity">
                            <div class="pro-qty">
                                ...
                                <input type="number" value="{{ $item->soluong }}" min="1"
                                    max="{{ $item->product->soluong }}" class="quantity-input"
                                    data-id="{{ $item->id_sp_giohang }}" data-original-value="{{ $item->soluong }}"
                                    onchange="handleQuantityChange(this)" readonly>
